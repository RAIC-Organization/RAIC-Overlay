...
- [ ] T007 [P] Define core types in `src-tauri/src/plugins/types.rs` per data-model.md: `PluginId`, `PluginRegistry`, `RegisteredPlugin`, `AvailableUpdate`, `Manifest`, `EntryPoint`, `Sidecar`, `SidecarPlatformBinary`, `Permission` enum, `PluginInstance` (runtime), `SidecarProcess` (runtime), `PluginState`, `JsonRpcRequest`, `JsonRpcResponse`, `JsonRpcError`, `RpcErrorCode` enum (with the catalogue from jsonrpc-v1.md)
- [ ] T008 Implement plugins-data-dir resolver in `src-tauri/src/plugins/registry.rs::plugins_dir(app: &AppHandle) -> PathBuf` returning `<app_data_dir>/plugins/`, ensuring the directory exists on first call
- [ ] T009 Implement `PluginRegistry` read/write in `src-tauri/src/plugins/registry.rs` (load_registry, save_registry, atomic write via tempfile + rename) reading `plugins/registry.json`
- [ ] T010 [P] Implement plugin-tagged log helper in `src-tauri/src/plugins/types.rs::PluginLog` (thin wrappers around `log::info!/warn!/error!` that always prepend `[plugin=<id>]`)
- [ ] T011 Register custom protocol handler `tauri://plugin/<id>/<path>` in `src-tauri/src/plugins/runtime/protocol.rs` that maps to files under `plugins/<id>/<version>/` (validate path is under the install dir, reject `..`)
- [ ] T012 Create the bootstrap script `src-tauri/resources/plugin-bootstrap.js` with skeleton `window.raic = { protocol: 1, pluginId, pluginVersion, windowId, rpc, subscribe, theme }` plus the JSON-RPC client (use `window.__TAURI_INTERNALS__.invoke('plugin_rpc', { plugin_id, request })` for requests, listen on `WebviewWindow` event `raic:event` for pushes); embed as `include_str!` in `src-tauri/src/plugins/runtime/window.rs`
- [ ] T013 Implement `WebviewWindowBuilder` factory in `src-tauri/src/plugins/runtime/window.rs::create_plugin_window` that takes `(app, plugin_id, version, entry_url, window_label, width, height)`, calls `.initialization_script(bootstrap_with_context)` substituting `pluginId`/`pluginVersion`/`windowId`/`theme` placeholders into the template
- [ ] T014 Add the single Tauri command `plugin_rpc(plugin_id: String, request: JsonRpcRequest) -> JsonRpcResponse` in `src-tauri/src/plugins/rpc/mod.rs` and register it in `src-tauri/src/lib.rs`'s `invoke_handler`; for now it returns `MethodNotFound` for all methods (handlers wired per story)
- [ ] T015 [P] Implement dispatcher skeleton in `src-tauri/src/plugins/rpc/dispatch.rs::dispatch(plugin_id, request) -> RpcResult` matching on `request.method` (only `permissions.list` and `permissions.has` wired here as they need no permission and are useful for early smoke tests)
- [ ] T016 [P] Implement permission check in `src-tauri/src/plugins/rpc/permissions.rs::require(plugin_id, perm) -> Result<(), RpcError>` reading from the registry's `granted_permissions`
